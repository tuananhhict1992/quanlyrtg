...
Mã cập nhật Google OAuth và điều khiển hàng đợi đã lưu vào AI Studio, đẩy lên GitHub `main` (commit `b26a3c8`) và triển khai thành công. Cloud Run project `distributed-aura-kf6jr`, service `quanlyrtg`, revision `quanlyrtg-00002-znd` lúc 11:53:51 ngày 26/09/2026 (UTC+7) nhận 100% lưu lượng. `/api/health` trả 200, `/api/me` không xác thực trả 401, callback OAuth mới khi thiếu phiên/code trả 400 đúng thiết kế. Trang đăng nhập production tải được. Preview dùng phiên admin thật đã tải Google Sync, đọc hàng đợi từ PostgreSQL; nút kết nối đang khóa đúng khi chưa có OAuth client. Đã sửa cảnh báo ảnh đại diện rỗng ở màn hình phân quyền. App cũ “Quản trị Nhân sự & Vận hành Nội bộ” đã ngừng xuất bản theo xác nhận của chủ tài khoản để giải phóng hạn mức; không thay đổi gói thanh toán. AI Studio có lúc còn hiển thị mốc Last published cũ sau cập nhật; đối chiếu Revision History và API thực tế. Cần hoàn tất OAuth client, cấp quyền và thử sync thật trước khi xác nhận Google Sync hoạt động.
...
Chưa có DATABASE_URL, Google credentials, folder ID hay spreadsheet ID trong môi trường local. AI Studio đã có DATABASE_URL trong Secrets. Drive API và Sheets API đã bật; OAuth branding RTG System đã tạo, client/cấp quyền thực còn chờ hoàn tất. Mã mới hỗ trợ Gmail OAuth với `openid email drive.file`, token mã hóa và tự tạo kho RTG, giữ phương án service account cho Shared Drive. Chưa xác nhận upload/sync Google thật, quota/SMTP. RUN_SYNC_WORKER hiện tắt trong AI Studio; bản mới bổ sung xử lý tối đa 25 job khi admin giữ trang Google Sync mở, chưa có lịch worker tự động Cloud Run. Các thông báo trùng đã lưu trước đây chưa được xử lý trên DB thật. Preview có thể báo lỗi websocket Vite trong lúc máy chủ khởi động lại; không coi Preview là bản production đã kiểm thử toàn bộ.
